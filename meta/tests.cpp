...
int main()
{
    ...
    testRegOp<void>(DDRA, 0, concat_t<sort_t<build_t<A0, A2, A4, A6>>>().ddr_set, 0b01010101);
    testRegOp<void>(DDRB, 0, concat_t<sort_t<build_t<B0, B2, B4, B6>>>().ddr_set, 0b01010101);
    testRegOp<void>(DDRC, 0, concat_t<sort_t<build_t<C0, C2, C4, C6>>>().ddr_set, 0b01010101);
    testRegOp<void>(DDRD, 0, concat_t<sort_t<build_t<D0, D2, D4, D6>>>().ddr_set, 0b01010101);

    cout << "Test: DDR, set, single group, default has pattern" << endl;
    testRegOp<void>(DDRA, 0b10101010, concat_t<sort_t<build_t<A0, A2, A4, A6>>>().ddr_set, 0xFF);
    testRegOp<void>(DDRB, 0b10101010, concat_t<sort_t<build_t<B0, B2, B4, B6>>>().ddr_set, 0xFF);
    testRegOp<void>(DDRC, 0b10101010, concat_t<sort_t<build_t<C0, C2, C4, C6>>>().ddr_set, 0xFF);
    testRegOp<void>(DDRD, 0b10101010, concat_t<sort_t<build_t<D0, D2, D4, D6>>>().ddr_set, 0xFF);

    cout << "Test: DDR, clear, single group, default has pattern" << endl;
    testRegOp<void>(DDRA, 0b10101010, concat_t<sort_t<build_t<A1, A3, A5, A7>>>().ddr_clear, 0);
    testRegOp<void>(DDRB, 0b10101010, concat_t<sort_t<build_t<B1, B3, B5, B7>>>().ddr_clear, 0);
    testRegOp<void>(DDRC, 0b10101010, concat_t<sort_t<build_t<C1, C3, C5, C7>>>().ddr_clear, 0);
    testRegOp<void>(DDRD, 0b10101010, concat_t<sort_t<build_t<D1, D3, D5, D7>>>().ddr_clear, 0);

    cout << "Test: DDR, clear, single group, default ist filled" << endl;
    testRegOp<void>(DDRA, 0xFF, concat_t<sort_t<build_t<A1, A3, A5, A7>>>().ddr_clear, 0b01010101);
    testRegOp<void>(DDRB, 0xFF, concat_t<sort_t<build_t<B1, B3, B5, B7>>>().ddr_clear, 0b01010101);
    testRegOp<void>(DDRC, 0xFF, concat_t<sort_t<build_t<C1, C3, C5, C7>>>().ddr_clear, 0b01010101);
    testRegOp<void>(DDRD, 0xFF, concat_t<sort_t<build_t<D1, D3, D5, D7>>>().ddr_clear, 0b01010101);
    ...
    testMultiRegOp<void>(
            DDRA, DDRB, DDRC, DDRD,
            0, 0, 0, 0,
            concat_t<sort_t<build_t<A0, B0, C0, D0, A2, B2, C2, D2, A4, B4, C4, D4, A6, B6, C6, D6>>>().ddr_set,
    ...
    testMultiRegOp<void>(
            DDRA, DDRB, DDRC, DDRD,
            0b10101010, 0b10101010, 0b10101010, 0b10101010,
            concat_t<sort_t<build_t<A1, B1, C1, D1, A3, B3, C3, D3, A5, B5, C5, D5, A7, B7, C7, D7>>>().ddr_clear,
            0, 0, 0, 0);
